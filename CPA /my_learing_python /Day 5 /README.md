...
#### **✍️ 문제 1. 데이터프레임 생성 및 결측치 확인**

  * **요구사항**:
    다음 Python 코드를 사용하여 `df_data` 데이터프레임을 생성하고, 각 컬럼별 결측치 개수를 출력하세요.
    ```python
    import pandas as pd
    import numpy as np
    data = {'A': [1, 2, np.nan, 4],
...
#### **✍️ 문제 2. 특정 조건의 결측치 삭제**

  * **요구사항**:
    문제 1에서 생성한 `df_data` 데이터프레임을 복사한 후, 'A' 컬럼 또는 'B' 컬럼 중 **어느 하나라도** 결측치가 있는 행을 제거하고 결과를 출력하세요.
    ```python
    import pandas as pd
    import numpy as np
    data = {'A': [1, 2, np.nan, 4],
            'B': [np.nan, 6, 7, np.nan],
            'C': ['x', 'y', 'z', 'w']}
    df_data = pd.DataFrame(data) # 문제 1과 동일한 df_data 사용
    ```
  * **가이드**:
      * 데이터프레임 복사는 `df.copy()`를 사용하세요.
      * `df.dropna(subset=['컬럼1', '컬럼2'], how='any')`를 사용하세요.
...
#### **✍️ 문제 3. 결측치 유형별 대체**
  ...
  * **요구사항**:
    문제 1에서 생성한 `df_data` 데이터프레임을 다시 초기화(재생성)한 후, 다음 지시에 따라 결측치를 대체하고 결과를 출력하세요.
    1.  'A' 컬럼의 결측치를 해당 컬럼의 **평균값**으로 채우세요.
    2.  'B' 컬럼의 결측치를 바로 **이전(상위) 행의 값**으로 채우세요. (첫 행이 NaN일 경우 그대로 남을 수 있음)
  * **가이드**:
    1.  `df['컬럼'].fillna(df['컬럼'].mean())` 사용.
    2.  `df['컬럼'].fillna(method='ffill')` 사용.
    <!-- end list -->
      * **주의**: 각 단계별로 원본 `df_data`가 변경되므로, 다음 단계로 넘어가기 전에 `df_data`를 다시 생성하거나 `copy()`를 사용하여 작업하세요.
...
#### **✍️ 문제 4. 중복값 확인 및 조건부 제거**

  * **요구사항**:
    다음 `sales_transactions` 데이터프레임을 생성하세요.
    ```python
    import pandas as pd
    data = {'TransactionID': [101, 102, 101, 103, 102, 104],
            'CustomerID': ['A001', 'A002', 'A001', 'A003', 'A002', 'A001'],
            'Date': ['2023-01-01', '2023-01-01', '2023-01-01', '2023-01-02', '2023-01-01', '2023-01-02'],
            'Amount': [100, 200, 150, 300, 250, 120]}
    sales_transactions = pd.DataFrame(data)
    ```
    1.  'TransactionID'와 'Date' 컬럼을 기준으로 중복된 행이 있는지 확인하고, 그 개수를 출력하세요.
    2.  'CustomerID'와 'Date'가 동일한 경우를 중복으로 간주하고, 'Amount'가 가장 작은 행만 남기고 나머지 중복된 행을 제거한 결과를 출력하세요.
  * **가이드**:
    1.  `df.duplicated(subset=['컬럼1', '컬럼2']).sum()` 사용.
    2.  `sort_values()`로 'Amount'를 **오름차순** 정렬한 후, `drop_duplicates(subset=['컬럼1', '컬럼2'], keep='first')`를 사용하세요.
