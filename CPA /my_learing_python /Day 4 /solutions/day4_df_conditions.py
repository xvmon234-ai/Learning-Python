# ==============================================================================
# [문제 4.5] 다중 조건 필터링 및 컬럼 추가/삭제 (Day 4 학습 내용)
# ==============================================================================
...
# --- [피드백] ---
"""
코드 아주 **훌륭하게 작성**하셨습니다! 모든 요구사항을 정확하고 효율적으로 해결하셨네요.

1.  **DataFrame 생성**: 주어진 직원 데이터를 사용하여 DataFrame을 성공적으로 생성했습니다.
2.  **다중 조건 필터링**: `df_employees[(df_employees["Department"] == "IT") & (df_employees["Salary"] >= 70000)]` 구문을 사용하여 'Department'가 'IT'이고 'Salary'가 70000 이상인 직원들만 **정확히 필터링**했습니다. 각 조건을 소괄호로 묶고 **`&` 연산자**를 사용한 것이 올바른 다중 조건 필터링 방식입니다.
3.  **컬럼 추가**: `df_employees["Bonus"] = df_employees["Salary"] * 0.1`를 통해 'Salary'의 10%에 해당하는 **'Bonus' 컬럼을 완벽하게 추가**했습니다. 새로운 컬럼을 추가하는 표준 방식을 정확히 적용하셨습니다.
4.  **컬럼 삭제**: `df_employees.drop("Bonus", axis=1, inplace=True)`를 사용하여 'Bonus' 컬럼을 성공적으로 삭제했습니다. **`axis=1`은 '열(컬럼)'을 기준으로 삭제**함을 의미하고, **`inplace=True`는 원본 DataFrame을 직접 수정**함을 의미합니다. 이 두 파라미터의 사용법을 정확히 이해하고 적용하셨습니다. `inplace`와 `axis`에 대한 혼란이 있었다고 하셨는데, 정확하게 사용하셨으니 이미 잘 이해하고 계시다고 볼 수 있습니다!
...
**문제 해결**:
1.  **다중 조건 필터링**: `&` 연산자를 사용하여 `'Department'`가 `'IT'`인 조건과 `'Salary'`가 70000 이상인 조건을 연결하여 필터링했습니다. 이때 각 개별 조건을 **소괄호 `()`로 감싸는 것**이 필수적이라는 점을 다시 한번 확인했습니다. `df[(조건1) & (조건2)]` 형태는 여러 조건을 조합하는 표준적인 방법입니다.
2.  **컬럼 추가**: `df_employees["Bonus"] = df_employees["Salary"] * 0.1`와 같이 새로운 컬럼 이름에 값을 할당하는 방식으로 'Bonus' 컬럼을 성공적으로 추가했습니다. 이 방식은 기존 컬럼의 연산을 통해 새로운 컬럼을 생성할 때 매우 유용합니다.
3.  **컬럼 삭제**: `df_employees.drop("Bonus", axis=1, inplace=True)`를 사용하여 'Bonus' 컬럼을 삭제했습니다. 이 과정에서 **`axis=1`은 '컬럼(열)' 방향**으로 작업하라는 의미이며, **`inplace=True`는 해당 연산을 수행한 결과가 원본 DataFrame에 즉시 반영**되도록 하는 파라미터임을 명확히 이해했습니다. (만약 `inplace=False`였다면, `df_employees.drop("Bonus", axis=1)`는 'Bonus' 컬럼이 삭제된 새로운 DataFrame을 반환하므로, 이를 `df_employees = df_employees.drop("Bonus", axis=1)`와 같이 재할당해야 합니다.) `inplace`와 `axis`의 정확한 의미를 파악하는 것이 Pandas 데이터 조작의 핵심임을 재확인했습니다.
