...
**문제 해결**:
1.  **DataFrame 생성 및 인덱스 설정**: `pd.read_csv` 함수를 사용하여 데이터를 로드함과 동시에 `.set_index("StudentID")` 메서드 체이닝을 통해 'StudentID' 컬럼을 DataFrame의 인덱스로 지정했다. 이를 통해 라벨 기반의 데이터 접근이 가능해졌다.
